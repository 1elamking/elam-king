...
SEARCH_ACTIONS = [
    ...
    {"type": "click",  "selector": {"type": "css", "value": "#ctl00_ContentPlaceHolder1_as1_lstCounty > li:nth-child(21) > label"}},
...
def is_valid_search(html):
    """County=El Paso ve tarih doğru mu kontrol et."""
...
def do_search():
    for attempt in range(1, SEARCH_RETRIES + 1):
        ...
        html = decodo_get(
            "https://www.publicnoticecolorado.com/default.aspx",
...
def fetch_detail(detail_url, notice_id):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if "reCAPTCHA" in html:
            ...
            post_url = f"https://www.publicnoticecolorado.com/(S({form_sid}))/Details.aspx?SID={form_sid}&ID={notice_id}"
...
for page in range(2, total_pages + 1):
    ...
    html_next = decodo_get(
        f"https://www.publicnoticecolorado.com/(S({sid}))/Search.aspx",
        browser_actions=[
...
for i, notice_id in enumerate(all_ids, 1):
    ...
    if pdf_href:
        try:
            pdf_href_sid = re.search(r'SID=([a-z0-9]+)', pdf_href).group(1)
            session_part = pdf_href_sid[:24]
            filename     = re.search(r'FileName=(.+)', pdf_href).group(1)
            pdf_url      = f"https://www.publicnoticecolorado.com/(S({session_part}))/PDFDocument.aspx?SID={pdf_href_sid}&FileName={filename}"
...
print(f"notices.csv kaydedildi.")
